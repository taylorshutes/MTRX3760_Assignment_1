...
if (last_colour_ == "red") {
skip_right_ = 1; // skip the very next right opportunity
RCLCPP_INFO(this->get_logger(), "ColourDetector: RED → set skip_right_=1 (one-shot)");
} else if (last_colour_ == "green") {
// Explicitly allow rights again when seeing green
if (skip_right_ != 0) {
RCLCPP_INFO(this->get_logger(), "ColourDetector: GREEN → clear skip_right_");
}
skip_right_ = 0;
}
}
