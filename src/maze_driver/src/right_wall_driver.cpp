...
geometry_msgs::msg::TwistStamped RightWallDriver::compute_command()
{
  ...
  if (std::isnan(front_distance_) || std::isnan(right_distance_) || std::isnan(left_distance_) ||
      front_distance_ == 0.0 || right_distance_ == 0.0 || left_distance_ == 0.0) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                        "Waiting for valid LiDAR data...");
    return cmd;  // do nothing this loop
  }
